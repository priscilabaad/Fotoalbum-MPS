...
## 8. Onde adicionar as fotos de teste

Abra `js/data.js` e edite o bloco `PHOTOS_MOCK`. Cada foto segue o formato:

```json
{ "id": "001", "nome": "IMG_0001.jpg", "url": "https://...", "numero": "001" }
```

Por padrão o projeto gera **60 fotos de exemplo** usando o serviço `picsum.photos`
(ajuste no `Array.from({ length: 60 })` do topo do bloco — pode aumentar para
100, 200, o quanto quiser). Para usar fotos reais de teste, troque o valor de
`url` pelo endereço de cada imagem (pode ser um link de imagem hospedada, ou
colocar os arquivos em `img/` e usar `url: "img/foto-001.jpg"`).

O arquivo `data/photos.example.json` mostra esse mesmo formato isoladamente,
caso prefira manter as fotos num JSON separado no futuro.
